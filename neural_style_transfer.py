...
#content_layer = ['conv_7']
#style_layer = ['conv_1', 'conv_3', 'conv_6', 'conv_9']
...
class Content_loss(nn.Module):
    ...
    def forward(self, f_current):
        #print(f_current.size(), self.P.size(), f_current.size() != self.P.size())
        if f_current.size() == self.P.size():
...
def image_loader(s_n, c_n, batch_s, image_size):
    img_c = Image.open(c_n) 
    #transforms.functional.resize(Image.open(c_n), (image_size, image_size))
    img_c = transforms.functional.to_tensor(img_c)
